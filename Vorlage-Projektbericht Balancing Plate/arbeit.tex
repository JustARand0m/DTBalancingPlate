...
Bei der Implementierung wird als Verbindung zur Touch Plate ein XMC4500-Mikro-controller verwendet.
...
In diesem Projekt sind die Pins der Touch Plate wie folgt mit dem XMC verbunden:
...
\caption{Verbindungen der Pins der Touch Plate mit dem XMC}
\end{table}
...
Die GPIO-Pins sind beim Start des Programms so eingestellt, dass erst der x-Wert ausgelesen wird und daraufhin der y-Wert. Sind beide Werte vorhanden, so werden die beiden Daten in einem UART Frame verpackt und dann ein UART-Transmit ausgelöst.\newline
...
\subsubsection{Probleme} \label{subsec:Probleme}
