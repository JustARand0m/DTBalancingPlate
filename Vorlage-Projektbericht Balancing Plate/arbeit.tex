...
Um die Bitfehler zu erkennen wurde ein CRC-Check implementiert, welcher über ein Generatorpolynom über das
gesammte Packet ermittelt wurde. Für die Implementierung wurde eine CRC-Tabelle benutzt diese hat den Vorteil,
...
Tabelle ist die Performance, da die Rechnung komplett mehr Performance ziehen würde. Der CRC-Check ist momemtan nur in
der Richtung vom XMC zum Raspberry Pi implementiert. Vom Touchpanel des XMCs zum PI wurde nicht umbedingt ein CRC benötigt,
da auf dem PI ein Kalman filter davorgeschalen wurde der diese Fehler ausgleicht. 
Der CRC könnte hier noch implementiert werden.

\subsubsection{Probleme}
Die größten Probleme beim UART war die Dave4 Dokumentation des XMCs. Online findet man ein sehr detailiertes Manuel zum 
XMC4500, jedoch nahezu keine Dokumentation wie Dave4 Apps funktioneren. Die OTH Rechner haben bei der Dave4 IDE keine 
Help Manuels was das ganze noch erschwehrt. Die Lösung dieses Problems ware die Help Manuels auf Dave4 manuel 
nachzuinstallieren. In diesem git es sehr viele nützliche Informationen wie UART eigentlich mittels der App funktioniert.

\subsubsection{Ausbick}
Folgende Punkte waren auf grund von Zeit nichtmehr möglich zum implementieren.
Der CRC-Check ist momentan aus zeitlichen gründen in nur eine Richtung implementiert, es ist zwar nicht nötig, wegen des
Kalman Filters aber, es wäre trozdem nicht schlecht in Richtung vom XMC zum PI einen CRC check zu haben. 
...
in der Endversion ist, schon gut genug gestest, sodass dieser ungetestet Code nicht nötige war.
% ----------------------------------------------------------------------------------
...
die ganze Zeit durch diese Loops Blockiert. Um dieses Verhalten zu umgehen wurden alle Listing-Loops in eigene Threads
verschoben. Da die Anwendung zentral von einem Button gesteuert wird, welcher zwischen den Modus zum Regeln und dem 
Modus zum manuel mit der App steuern, gesteuert wird, befindet sich die Ansteuerung des Buttons im main thread.
...
Mutex der Zugriff auf die Daten abgeschert. Z.B. kann nur entweder UART schreiben oder der Benutzer die Daten abfragen,
